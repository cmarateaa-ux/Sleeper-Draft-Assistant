...
async function buildRecommendation(draftId) {
  ...
  const candidates = rows.map(([id, row]) => {
    ...
    const adpValue = Math.max(0, 100 - adp);
    const projectionValueScore = Math.min(40, market.points / 8);
    const value = adpValue + projectionValueScore;
...
const server = http.createServer(async (req, res) => {
  try {
    ...
    if (url.pathname === "/api/health") {
      res.writeHead(200, { "content-type": "application/json", "cache-control": "no-store" });
      res.end(JSON.stringify({ ok: true, build: "live-recommendations-2026-08-31-2" })); return;
    }
