...
const BUILD="sleeper-adp-reconciliation-2026-08-31-24";
...
async function sleeperAdp(scoring,year){
  // This is Sleeper's published projection-feed ADP. It is NOT the ADP label
  // displayed in a specific live draft room, so the UI must never call it
  // verified live draft-board ADP.
...
const candidates=raw.map(x=>{
  ...
  // ADP is a market-timing signal, not player value. Bound its influence so a
  // later ADP cannot create an artificial giant "steal" score.
...
const adpLabel=adpMap.size?"Sleeper published ADP feed":"unavailable";
return{build:BUILD,scoringType:scoring,teams,draftStatus:draft.status,currentPickNo:current,currentSlot:slot(current,teams,type),userSlot,userPickNo:userPick,nextUserPick,picksUntilUser:userPick?userPick-current:null,availableCount:available,roster,recentPicks,marketSource:adpLabel,adpSource:"sleeper_projection_feed",recommendation:best?{name:best.name,position:best.position,team:best.team,adp:best.adp,points:null,confidence,nextPick:nextUserPick,survivalPct:nextUserPick?(best.adp<=nextUserPick?100:Math.max(0,Math.min(100,Math.round(100-(best.adp-nextUserPick)*9)))):null,reason:`Published Sleeper ADP ${best.adp.toFixed(1)} · pick ${userPick??"—"} · ${best.adpVsPick>=0?`about ${Math.round(best.adpVsPick)} spots later than this pick`:`about ${Math.round(Math.abs(best.adpVsPick))} spots earlier than this pick`}. ${best.starterNeed>0?`Fills an immediate ${best.position} starting need.`:"Roster priority and pick value both considered."} This is published Sleeper feed ADP, not verified live draft-room ADP.`,plan}:null,alternatives:candidates.slice(1,5).map(x=>({name:x.name,position:x.position,team:x.team,adp:x.adp,points:null,survivalPct:nextUserPick?(x.adp<=nextUserPick?100:Math.max(0,Math.min(100,Math.round(100-(x.adp-nextUserPick)*9)))):null,adpVsPick:userPick?userPick-x.adp:null})),nextTurnTargets,nextPickDistance:userPick&&nextUserPick?nextUserPick-userPick:null}}
