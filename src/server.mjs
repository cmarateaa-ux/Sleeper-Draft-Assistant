...
const BUILD="sleeper-adp-reconciliation-2026-08-31-27";
...
const needs={QB:Number(draft.settings?.slots_qb??1),RB:Number(draft.settings?.slots_rb??2),WR:Number(draft.settings?.slots_wr??2),TE:Number(draft.settings?.slots_te??1)},flexSlots=Number(draft.settings?.slots_flex??draft.settings?.slots_wrt??2),adpMap=await sleeperAdp(scoring,draft.season??new Date().getFullYear());
...
const candidates=raw.map(x=>{
  ...
  // Once the starting lineup is filled, additional players at that position
  // should require a meaningful market edge. This prevents the assistant from
  // repeatedly recommending WR simply because WR has many strong ADPs.
  const rbWrStarterSlots=needs.RB+needs.WR+flexSlots;
  const rbWrCount=roster.RB+roster.WR;
  const surplusPenalty=(x.position==="WR"&&roster.WR>=needs.WR+Math.max(0,flexSlots-1))
    ?Math.min(18,6+(roster.WR-(needs.WR+Math.max(0,flexSlots-1)))*6)
    :(x.position==="RB"&&roster.RB>=needs.RB+1)?8:0;
  ...
  const score=marketValue+startingNeed+flexNeed+qbDepthPenalty+teDepthPenalty-surplusPenalty-reachPenalty;
  return{...x,score,starterNeed,adpVsPick,surplusPenalty};
