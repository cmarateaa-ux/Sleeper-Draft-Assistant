...
function projection(seasonRow, marketRow, scoring) {
  ...
  const adp=scoring==="ppr"?(b.adp_ppr??a.adp_ppr??marketRow?.adp_ppr??seasonRow?.adp_ppr??b.adp_dd_ppr??a.adp_dd_ppr??marketRow?.adp_dd_ppr??seasonRow?.adp_dd_ppr):scoring==="half_ppr"?(b.adp_half_ppr??a.adp_half_ppr??marketRow?.adp_half_ppr??seasonRow?.adp_half_ppr??b.adp_dd_half_ppr??a.adp_dd_half_ppr??marketRow?.adp_dd_half_ppr??seasonRow?.adp_dd_half_ppr):(b.adp_std??a.adp_std??marketRow?.adp_std??seasonRow?.adp_std??b.adp_dd_std??a.adp_dd_std??marketRow?.adp_dd_std??seasonRow?.adp_dd_std);
...
async function recommend(draftId) {
  const [draft,picks]=await Promise.all([sleeper(`/draft/${encodeURIComponent(draftId)}`),sleeper(`/draft/${encodeURIComponent(draftId)}/picks`)]);
  const cacheKey=`${draftId}:${draft.status}:${picks.length}`;
  const cached=recommendationCache.get(draftId);
  if(cached?.key===cacheKey && Date.now()-cached.at<1500) return cached.data;
  if(recommendationInflight.has(draftId)) return recommendationInflight.get(draftId);
  const work=(async()=>{
    ...
    const candidates=tier.map(x=>{
      ...
      const fall=userPick&&x.adp!=null?Math.max(0,Math.min(10,(adp-userPick)*0.35)):0, reach=userPick&&x.adp!=null?Math.max(0,userPick-adp-5)*2.5;
    ...
    return {scoringType:scoring,teams,draftStatus:draft.status,currentPickNo:current,currentSlot:slot(current,teams,type),userSlot,userPickNo:userPick,nextUserPick,picksUntilUser:userPick?userPick-current:null,availableCount:available,roster,recentPicks,recommendation:best?{name:best.name,position:best.position,team:best.team,adp:best.adp,points:best.points,confidence,nextPick:nextUserPick,survivalPct:best.survivalPct,reason:`ADP ${best.adp?.toFixed(1)??"—"} · projected ${best.points.toFixed(1)} pts · market tier, projection value and your next pick considered.`,plan}:null,alternatives:candidates.slice(1,5).map(x=>({name:x.name,position:x.position,team:x.team,adp:x.adp,points:x.points,survivalPct:x.survivalPct})),nextTurnTargets,nextPickDistance:userPick&&nextUserPick?nextUserPick-userPick:null};
  })();
  recommendationInflight.set(draftId,work);
  try { const data=await work; recommendationCache.set(draftId,{key:cacheKey,at:Date.now(),data}); return data; } finally { recommendationInflight.delete(draftId); }
}
