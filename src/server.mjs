...
const BUILD="sleeper-adp-eligibility-2026-08-31-20";

async function sleeper(pathname){const r=await fetch(`${sleeperBase}${pathname}`);if(!r.ok)throw new Error(`Sleeper returned ${r.status} · ${await r.text()}`);return r.json()}
...
async function sleeperAdp(scoring,year){
  const cacheKey=`${scoring}-${year}`;
  const cached=sleeperAdpCache.get(cacheKey);
  if(cached&&Date.now()-cached.at<30*60*1000)return cached.data;
  try{
    const r=await fetch(`https://api.sleeper.com/projections/nfl/${year}?season_type=regular`,{headers:{"user-agent":"Sleeper-Draft-Assistant/0.1"}});
    ...
    const map=new Map();
    for(const p of Array.isArray(json)?json:[]){const id=String(p.player_id??"");const adp=Number(p[field]);if(id&&Number.isFinite(adp))map.set(id,adp)}
    sleeperAdpCache.set(cacheKey,{at:Date.now(),data:map});
    return map;
  }catch{return new Map()}
}
async function ffcAdp(scoring,teams,year){const format=scoring.includes("ppr")?"ppr":"standard",cacheKey=`${format}-${teams}-${year}`,cached=ffcCache.get(cacheKey);if(cached&&Date.now()-cached.at<6*60*60*1000)return cached.data;try{const r=await fetch(`https://fantasyfootballcalculator.com/api/v1/adp/${format}?teams=${teams}&year=${year}&position=all`,{headers:{"user-agent":"Sleeper-Draft-Assistant/0.1"}});if(!r.ok)throw new Error(`FFC returned ${r.status}`);const json=await r.json(),map=new Map;for(const p of json.players??[]){const adp=Number(p.adp);if(p.name&&Number.isFinite(adp)&&adp>0&&adp<300)map.set(keyName(p.name),adp)}ffcCache.set(cacheKey,{at:Date.now(),data:map});return map}catch{return new Map}}
...
const [sleeperMap,ffcMap]=await Promise.all([sleeperAdp(scoring,draft.season??new Date().getFullYear()),ffcAdp(scoring,teams,draft.season??new Date().getFullYear())]);
const raw=Object.entries(players).map(([id,pl])=>{
  ...
  if(!name)return null;
  const sleeperValue=sleeperMap.get(id);
  // Sleeper uses 999 as the explicit "not being drafted" sentinel. If Sleeper
  // knows the player and says 999, do not resurrect him with an outside ADP.
  if(sleeperValue!=null&&sleeperValue>=999)return null;
  const adp=sleeperValue??ffcMap.get(keyName(name));
  if(adp==null||!Number.isFinite(adp)||adp>=999)return null;
  return{id,name,position:pos,team:String(pl.team),adp,points:null,marketSource:sleeperValue!=null?"Sleeper":"FFC fallback"};
}).filter(Boolean);
const round=userPick?Math.ceil(userPick/teams):1;
const candidates=raw.filter(x=>!userPick||x.adp<=userPick+30).map(x=>{const rosterCount=roster[x.position]??0,starterNeed=Math.max(0,(needs[x.position]??0)-rosterCount),startingNeed=starterNeed>0?(round<=3?Math.min(12,starterNeed*4):Math.min(18,starterNeed*6)):0,flexNeed=(x.position==="RB"||x.position==="WR")&&round<=8&&roster.RB+roster.WR<needs.RB+needs.WR+2?4:0,qbDepthPenalty=x.position==="QB"&&roster.QB>=needs.QB?-22:0,teDepthPenalty=x.position==="TE"&&roster.TE>=needs.TE?-10:0;
const marketValue=userPick?Math.max(0,Math.min(100,50+(userPick-x.adp)*2.4)):Math.max(0,100-x.adp),reachPenalty=userPick?Math.max(0,(userPick-x.adp-10)*1.5):0,score=marketValue*.78+startingNeed+flexNeed+qbDepthPenalty+teDepthPenalty-reachPenalty;return{...x,score,starterNeed}}).sort((a,b)=>b.score-a.score||a.adp-b.adp||a.name.localeCompare(b.name));
...
return{build:BUILD,scoringType:scoring,teams,draftStatus:draft.status,currentPickNo:current,currentSlot:slot(current,teams,type),userSlot,userPickNo:userPick,nextUserPick,picksUntilUser:userPick?userPick-current:null,availableCount:available,roster,recentPicks,marketSource:sleeperMap.size?"Sleeper ADP":"FFC fallback",recommendation:best?{name:best.name,position:best.position,team:best.team,adp:best.adp,points:null,confidence,nextPick:nextUserPick,survivalPct:best.adp<=nextUserPick?100:Math.max(0,Math.min(100,Math.round(100-(best.adp-nextUserPick)*9))),reason:`ADP ${best.adp.toFixed(1)} · ${best.starterNeed>0?`fills an immediate ${best.position} starting need`:`roster priority and pick value considered`}. ${best.marketSource}. Projection data is not being faked when unavailable.`,plan}:null,alternatives:candidates.slice(1,5).map(x=>({name:x.name,position:x.position,team:x.team,adp:x.adp,points:null,survivalPct:x.adp<=nextUserPick?100:Math.max(0,Math.min(100,Math.round(100-(x.adp-nextUserPick)*9)))})),nextTurnTargets,nextPickDistance:userPick&&nextUserPick?nextUserPick-userPick:null}}
