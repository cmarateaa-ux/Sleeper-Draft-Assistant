...
const BUILD="sleeper-adp-eligibility-2026-08-31-22";
...
async function sleeperAdp(scoring,year){
  ...
  try{
    ...
    for(const p of Array.isArray(rows)?rows:[]){
      const id=String(p.player_id??p.stats?.player_id??"");
      const rawAdp=Number(p[field]??p.stats?.[field]);
      if(id&&Number.isFinite(rawAdp)&&rawAdp>0&&rawAdp<999)map.set(id,rawAdp);
    }
