...
const BUILD="sleeper-adp-reconciliation-2026-08-31-23";
...
async function sleeperAdp(scoring,year){
  // Sleeper's published projection-feed ADP is useful for draft timing,
  // but it is not guaranteed to equal the number shown in a live draft room.
...
const candidates=raw.filter(x=>!userPick||x.adp<=userPick+30).map(x=>{
  const rosterCount=roster[x.position]??0;
  const starterNeed=Math.max(0,(needs[x.position]??0)-rosterCount);
  const startingNeed=starterNeed>0?(round<=3?Math.min(12,starterNeed*4):Math.min(18,starterNeed*6)):0;
...
return{build:BUILD,scoringType:scoring,teams,draftStatus:draft.status,currentPickNo:current,currentSlot:slot(current,teams,type),userSlot,userPickNo:userPick,nextUserPick,picksUntilUser:userPick?userPick-current:null,availableCount:available,roster,recentPicks,marketSource:adpMap.size?"Sleeper published week-1 ADP feed":"unavailable",adpSource:"sleeper_projection_feed",recommendation:best?{
  name:best.name,position:best.position,team:best.team,adp:best.adp,points:null,confidence,nextPick:nextUserPick,
  survivalPct:best.adp<=nextUserPick?100:Math.max(0,Math.min(100,Math.round(100-(best.adp-nextUserPick)*9))),
  reason:`ADP ${best.adp.toFixed(1)} · current pick ${userPick??"—"} · ${best.adpVsPick>=0?`about ${Math.round(best.adpVsPick)} spots later than this pick`:`about ${Math.round(Math.abs(best.adpVsPick))} spots earlier than this pick`}. ${best.starterNeed>0?`Fills an immediate ${best.position} starting need.`:"Roster priority and pick value both considered."} Published Sleeper ADP is used as a market-timing signal; it is not represented as verified live draft-room ADP.`,
  plan
}:null,alternatives:candidates.slice(1,5).map(x=>({name:x.name,position:x.position,team:x.team,adp:x.adp,points:null,survivalPct:x.adp<=nextUserPick?100:Math.max(0,Math.min(100,Math.round(100-(x.adp-nextUserPick)*9))),adpVsPick:userPick?userPick-x.adp:null})),nextTurnTargets,nextPickDistance:userPick&&nextUserPick?nextUserPick-userPick:null}}}}
