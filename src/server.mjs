...
const BUILD="sleeper-adp-eligibility-2026-08-31-21";
...
async function sleeperAdp(scoring,year){
  const field=scoring.includes("half")?"adp_dd_half_ppr":scoring.includes("standard")||scoring.includes("std")?"adp_dd_std":"adp_dd_ppr";
  ...
  try{
    ...
    for(const p of Array.isArray(rows)?rows:[]){const id=String(p.player_id??"");const rawAdp=Number(p[field]);if(id&&Number.isFinite(rawAdp)&&rawAdp>0&&rawAdp<999)map.set(id,rawAdp)}
...
return{build:BUILD,scoringType:scoring,teams,draftStatus:draft.status,currentPickNo:current,currentSlot:slot(current,teams,type),userSlot,userPickNo:userPick,nextUserPick,picksUntilUser:userPick?userPick-current:null,availableCount:available,roster,recentPicks,marketSource:adpMap.size?"Sleeper week-1 ADP":"unavailable",recommendation:best?{name:best.name,position:best.position,team:best.team,adp:best.adp,points:null,confidence,nextPick:nextUserPick,survivalPct:best.adp<=nextUserPick?100:Math.max(0,Math.min(100,Math.round(100-(best.adp-nextUserPick)*9))),reason:`ADP ${best.adp.toFixed(1)} · ${best.starterNeed>0?`fills an immediate ${best.position} starting need`:`roster priority and pick value considered`}. Sleeper ADP is being used directly. Projection data is not being faked when unavailable.`,plan}:null,alternatives:candidates.slice(1,5).map(x=>({name:x.name,position:x.position,team:x.team,adp:x.adp,points:null,survivalPct:x.adp<=nextUserPick?100:Math.max(0,Math.min(100,Math.round(100-(x.adp-nextUserPick)*9)))})),nextTurnTargets,nextPickDistance:userPick&&nextUserPick?nextUserPick-userPick:null}}
