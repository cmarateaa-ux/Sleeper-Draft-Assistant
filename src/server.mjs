...
async function buildRecommendation(draftId) {
  ...
  const week = await fetchJson(SLEEPER_PROJECTIONS, "?season_type=regular");
  ...
  return {
    ...
    recommendation: best ? {
      ...
      reason: best.adp ? `Sleeper ADP ${best.adp.toFixed(1)} · projected ${best.points.toFixed(1)} pts · roster need and pick-range value included.` : "Strong Sleeper projection with no current ADP signal.",
    } : null,
...
const server = http.createServer(async (req, res) => {
  try {
    ...
    if (url.pathname === "/api/health") { res.writeHead(200, { "content-type": "application/json", "cache-control": "no-store" }); res.end(JSON.stringify({ ok: true, build: "live-recommendations-2026-08-31-3" })); return; }
